...
namespace arris {
	namespace wechat {
		class agree_friend_request_msg :public iwxmsg {
		public:
			agree_friend_request_msg(DWORD eax) ://kAgreeFriendRequest
				type_(static_cast<int>(MsgType::kAgreeFriendRequest)),
				eax_(eax)
			{
				msgdispatch_ = std::make_unique<msgdispatch>();
			}
			~agree_friend_request_msg() = default;
		public:
